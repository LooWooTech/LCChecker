...
namespace LCChecker.Models
{
    public class DetectEngine
    {
        ...
        public DetectEngine(string region,List<Project> Projects)
        {
            ...
            //第8栏填写：否 第9栏为类型2 ,第11，12,13栏都填否 或者不填，那么17 28 32栏中至少有一个是有面积
            list.Add(new ConditionalRowRule()
            {
                Condition = rule2,
                Rule = new ConditionalRowRule()
                {
                    ...
                    Rule = new ConditionalRowRule()
                    {
                        Condition = new AndRule()
                        {
                            Rule1 = new CellRangeRowRule() { ColumnIndex = 10, Values = new[] { "否", "" } },
                            Rule2 = new AndRule()
                            {
                                Rule1 = new CellRangeRowRule() { ColumnIndex = 11, Values = new[] { "否", "" } },
                                Rule2 = new CellRangeRowRule() { ColumnIndex = 12, Values = new[] { "否", "" } }
                            }
                        },
                        Rule = new MultipleCellRangeRowRule() { 
